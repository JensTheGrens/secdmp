...
#  define htobe32(x)                                                           \
    (__extension__({                                                           \
      confidential static uint32_t _temp;                                      \
      _temp = (x);                                                             \
      ((_temp >> 24) & 0x000000FF) | ((_temp >> 8) & 0x0000FF00) |             \
          ((_temp << 8) & 0x00FF0000) | ((_temp << 24) & 0xFF000000);          \
    }))
...
#  define htobe64(x)                                                           \
    (__extension__({                                                           \
      confidential static uint64_t __temp;                                     \
      confidential static uint32_t __low;                                      \
      confidential static uint32_t __high;                                     \
      __temp = (x);                                                            \
...
inline static uint16_t load16(uint8_t *b) {
  confidential static uint16_t x;
  memcpy(&x, b, 2);
  return x;
}
...
inline static uint64_t load64(uint8_t *b) {
  confidential static uint64_t x;
  memcpy(&x, b, 8);
  return x;
}
