...
/** Below are all the processor configurations for the SecDMP paper: 1. Insecure Proteus+DMP
  * baseline, 2. SecDMP defense, 3. Selective disabling defense. 4. Permanent disabling defense
  * (without DMP), 5. ProSpeCT baseline with insecure DMP, 6. ProSpeCT+SecDMP, 7. Core to check
  * partitioning correctness
  */

object CoreDynamicProteusDmp {
  def main(args: Array[String]) {
    if (args.length != 1) {
      throw new IllegalArgumentException(
        "Incorrect configuration: provide the DMP type as command line argument"
      )
    }
    implicit val config: DynamicPipelineConfig = Config.secDmpConfig(args(0))
  ...
  def main(args: Array[String]) {
    ...
    SpinalVerilog(
      SoC.dynamic(
        ...
        extraDbusReadDelay = config.extraDbusReadDelay,
        applyDelayToIBus = config.applyDelayToIBus,
        extraPlugins = Seq(new MemoryPartitioning, new SecDmp)
      )
    )
  }
}

object CoreDynamicSelectiveDmpDisabling {
  def main(args: Array[String]) {
    if (args.length != 1) {
      throw new IllegalArgumentException(
        "Incorrect configuration: provide the DMP type as command line argument"
    ...
    SpinalVerilog(
      SoC.dynamic(
        ...
        extraDbusReadDelay = config.extraDbusReadDelay,
        applyDelayToIBus = config.applyDelayToIBus,
        extraPlugins = Seq(new SecurityController, new SelectivePrefetcherDisabling)
      )
    )
  }
}

object CoreDynamicPermanentDmpDisabling {
  def main(args: Array[String]) {
    implicit val config: DynamicPipelineConfig = Config.secDmpConfig("None")
...
object CoreDynamicProSpeCTSecDmp {
  def main(args: Array[String]) {
    if (args.length != 1) {
      throw new IllegalArgumentException(
        "Incorrect configuration: provide the DMP type as command line argument"
...
object CoreDynamicCheckPartition {
  def main(args: Array[String]) {
    implicit val config: DynamicPipelineConfig = Config.secDmpConfig("None")
