...
class Lsu(addressStages: Set[Stage], loadStages: Seq[Stage], storeStage: Stage)
  ...
  override def build(): Unit = {
    ...
    storeStage plug new Area {
      ...
      val secretPartitionViolation = False
      val exception_thrown =
        if (pipeline.hasService[SecretPartitionCheckerService]) Reg(Bool()).init(False) else null

      if (pipeline.hasService[SecretPartitionCheckerService]) {
        when(
          pipeline.service[PipelineTaintService].tainted(storeStage) && !pipeline
            .service[MemoryPartitioningService]
            .isInPartition(isStage = true, address = address)
        ) {
          secretPartitionViolation := True
        }

        when(isActive && secretPartitionViolation && !misaligned && !exception_thrown) {
          if (pipeline.hasService[TrapService]) {
            trap(TrapCause.StoreAddressMisaligned(address), storeStage)
      ...
      when(arbitration.isValid && !misaligned && !secretPartitionViolation) {
        when(isActive) {
