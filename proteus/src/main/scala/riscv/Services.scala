...
trait MemoryPartitioningService {

  /** Checks if the given address is part of the partition. The address's lowest `insignificantBits`
    * bits are ignored, i.e. if the address lies outside the region only with these lowest bits, the
    * function returns True. When calling this function from a specific pipeline stage, set
    * `isStage` to true, otherwise to false
    */
  def isInPartition(isStage: Boolean, address: UInt, insignificantBits: Int = 0): Bool
}

trait SecDmpService {}

object SecurityModeType extends SpinalEnum {
  val OFF = newElement()
  val MEDIUM = newElement()
  val HIGH = newElement()

  /** Returns True when left > right
    */
  def greaterThan(
      left: SpinalEnumCraft[SecurityModeType.type],
      right: SpinalEnumCraft[SecurityModeType.type]
  ): Bool = {
    (left === HIGH && right =/= HIGH) || (left === MEDIUM && right === OFF)
  }
}

trait SecurityService {

  /** Get the current security mode. When calling this function from a specific pipeline stage, set
    * `isStage` to true, otherwise to false
    */
  def getSecurityMode(isStage: Boolean): SpinalEnumCraft[SecurityModeType.type]
}

/** Disables DMPs when SecurityMode is MEDIUM, disables all prefetchers when SecurityMode is HIGH
  */
trait SelectivePrefetcherDisablingService {}

trait ProSpeCTService {}

trait SecretPartitionCheckerService {}
